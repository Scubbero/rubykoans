...
class AboutRegularExpressions < Neo::Koan
  ...
  def test_question_mark_means_optional
    ...
    assert_equal "a", "abbcccddddeeeee"[/az?/] # if "az" is not there "a" also works
  end
  ...
  def test_asterisk_means_zero_or_more
    ...
    # Guess: empty string?
    # A: nope it doesnt fail, altough it doesnt match something like azc with /ab*c/, 
    # source https://stackoverflow.com/questions/5084141/getting-the-star-regex-operator-to-fail
  end
  ...
  #
  # Why?
  # Because they take all they can (?)

  # ------------------------------------------------------------------

  def test_the_left_most_match_wins
    assert_equal "a", "abbccc az"[/az*/] # oh * doesnt' force the "z" so a matches as well
  end
  ...
  def test_slash_b_anchors_to_a_word_boundary
    assert_equal "vines", "bovine vines"[/\bvine./] # not "vine " because \b makes it only to match words
  end
  ...
  def test_variables_can_also_be_used_to_access_captures
    ...
    assert_equal "Gray", $1 # ?
    assert_equal "James", $2 # global variables, when a regex is evaluated they are reassigned
  end
  ...
  # A: an alternation allows to match whole words and character classes only single characters
